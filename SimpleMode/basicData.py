...
def BaseData(chatrooms_group, chatrooms_single,filename="basic_ana", start_time="1970-01-02", end_time=""):
    ...
    with open(f"{filename}.txt", "w+", encoding="utf-8") as f:
        f.write(f"好友总数：{len(chatrooms_single)}" + "个\n")
        f.write(f"群聊总数：{len(chatrooms_group)}" + "个\n")
        f.write(f"总共发出：{str(counter1)}" + "条\n")
        f.write(f"总共发出（好友）：{str(counter2)}" + "条\n")
        f.write(f"总共发出（群聊）：{str(counter5)}" + "条\n")
        f.write(f"总共接收：{str(counter3)}" + "条\n")
        f.write(f"总共接收（好友）：{str(counter4)}" + "条\n")
        f.write(f"总共接收（群聊）：{str(counter6)}" + "条\n")
...
def MostEmoji(chatrooms_group, chatrooms_single, filename="emoji_ranking", start_time="1970-01-02", end_time=""):
    ...
    for chatroom in chatrooms_all:
        sql = f"SELECT Message,CreateTime as num FROM {chatroom} WHERE Type=47 and Des=0 and CreateTime>=? and CreateTime<=?"
    ...
    for chatroom in chatrooms_single:
        sql = f"SELECT Message,CreateTime as num FROM {chatroom} WHERE Type=47 and Des=1 and CreateTime>=? and CreateTime<=?"
        Name = basicTool.GetName(chatroom)
        with basicTool.SqliteInit() as sqlite_cur:
            sqlite_cur.execute(sql,(str(start_time_stamp),str(end_time_stamp)))
            result = sqlite_cur.fetchall()
    ...
    with open(f"{filename}.txt", "w+", encoding="utf-8") as f:
        f.write(f"发出最多的表情包：{sorted_list_to[0][0]}" + "\n")
        f.write(f"共{str(sorted_list_to[0][1][0])}次" + "\n")
        f.write(
            f"聊天记录定位：微信名：{sorted_list_to[0][1][1]}，时间：{str(datetime.fromtimestamp(sorted_list_to[0][1][2]))}"
            + "\n\n"
        )

        f.write(f"接收最多的表情包：{sorted_list_from[0][0]}" + "\n")
        f.write(f"共{str(sorted_list_from[0][1][0])}次" + "\n")
        f.write(
            f"聊天记录定位：微信名：{sorted_list_from[0][1][1]}，时间：{str(datetime.fromtimestamp(sorted_list_from[0][1][2]))}"
            + "\n"
        )
...
def TypeAnalyse(chatrooms_single, filename="Type_ana", start_time="1970-01-02", end_time=""):
        ...
        for j in basicTool.GetData(i,["Type","Des"],start_time=start_time,end_time=end_time,Type=2):
            if j[1] == 0:
                if j[0] in single_type_counter_to:
                    ...
                    single_type_counter_to[j[0]] = 1
            elif j[0] in single_type_counter_from:
    ...
    pie.render(path=f"{filename}.html")
...
def RowAnalyse(chatrooms_single,filename="Row_ana", start_time="1970-01-02", end_time=""):
    ...
    x_axis = [
        f"{str(i + 1)},{basicTool.GetName(sorted_list[i][0])}"
    ...
    bar_top.render(path=f"{filename}.html")
        ...
        for i in chatrooms_single:
            for j in basicTool.GetData(i,["CreateTime","Message","Des","Type"],start_time=start_time,end_time=end_time,Type=2):
                ...
                if start<=time_array[3]<=end:
                    ...
                    f.write(f"{basicTool.GetName(i)},{str(j[2])},{CreateTime},{Message}" + "\n")
